...
export default function ClinicTicketsPage() {
  ...
  const assigneeOptions = useMemo(() => {
    const unique = Array.from(new Set(tickets.filter(t => t.visibility === 'public').map(t => t.assignee)));
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter(ticket => {
      ...
      // Solo ticket pubblici in questa vista
  ...
  const updateTicketStatus = (id: string, newStatus: typeof mockTickets[number]['status']) => {
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center">
              ...
              Ticket clinica · Pubblici
            </h1>
            <p className="text-gray-600 mt-1">
              Aperti dai miei colleghi • {filteredTickets.length} ticket{filteredTickets.length !== 1 ? 's' : ''}
            </p>
          </div>
                    ...
                    <tr key={ticket.id} className={`hover:bg-gray-50 transition-colors ${getRowColor(ticket)}`}>
                      ...
                      <td className="px-3 py-3 whitespace-nowrap text-center">
                        {editingStatusId === ticket.id ? (
                          <Select
                            value={ticket.status}
                            onChange={(e) => updateTicketStatus(ticket.id, e.target.value as typeof mockTickets[number]['status'])}
